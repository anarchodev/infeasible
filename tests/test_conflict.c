/* Golden test for the #98 conflictable-pair compile check (EPIC #154).
 *
 * Template-level, warning-severity (never fails a compile), two sides:
 *
 ...
 * different multi-valued values, merge-less `:=` — warn unless their
 * conditions provably exclude co-firing (complementary literal, different MV
 * constants, disjoint comparison intervals, disjoint #95 membership lists)
 * or the collision itself is impossible (distinct entity constants). A
 * writer can also collide with ITSELF when a scope variable is missing from
 * the effect's arguments and the assigned content varies with the binding.
 * Acceptance: a zero-warning story cannot take the contested step paths.
 *
 * JUDGMENT (the silently-REFUTED null made loud): complementary concluding
...
/* Compile and count diagnostics whose message contains `frag` (all must be
 * warnings — any error fails). Returns -1 on compile error. */
...
static int test_step_boolean(void)
{
    ...
    CHECK(nwarn(src, "conflicting effects on 'open'") == 1);
...
static int test_step_entity_disjoint(void)
{
    ...
    snprintf(src, sizeof src,
             ...
             "action clear:          causes ~tagged(b)\n");
    CHECK(nwarn(src, "conflicting effects on 'tagged'") == 1);
    return 0;
}
...
static int test_step_mv(void)
{
    ...
    CHECK(nwarn(src, "conflicting effects on 'mode'") == 1);
...
static int test_step_assign(void)
{
    ...
    CHECK(nwarn(src, "both assign (`:=`) 'hp'") == 1);
...
static int test_step_self(void)
{
    ...
    CHECK(nwarn(src, "more than once in one step") == 1);
